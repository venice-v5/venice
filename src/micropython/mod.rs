...
use self::raw::{
    CompiledModule, ModuleContext, NLR_REG_COUNT, NlrBuf, m_malloc, mp_call_function_0, mp_deinit,
    mp_make_function_from_proto_fun, mp_obj_print_exception, mp_plat_print, mp_raw_code_load_mem,
    mp_state_ctx, nlr_pop, nlr_push,
};
use crate::vcbc::Bytecode;
...
impl MicroPython {
    pub unsafe fn new() -> Self {
        unsafe {
            ...
            raw::gc_init(
                &raw mut __heap_start as *mut c_void,
                &raw mut __heap_end as *mut c_void,
            );
            raw::mp_init();
        }
        Self(())
    ...
    fn push_nlr<R>(&mut self, f: impl FnOnce() -> R) -> R {
        ...
        unsafe {
            if nlr_push(&raw mut nlr_buf) == 0 {
                ...
                mp_obj_print_exception(&raw const mp_plat_print, nlr_buf.ret_val);
            }
        }
    }
    ...
    pub fn exec_bytecode(&mut self, bytecode: Bytecode) {
        self.push_nlr(|| unsafe {
            ...
            mp_raw_code_load_mem(bytecode.data().as_ptr(), bytecode.data().len(), &raw mut cm);
