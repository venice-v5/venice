...
- [venice](./venice/venice): Contains the Python package for Venice. In CI, Micropython and `modvenice` are cross-compiled to the brain's target triple, and `objcopy`ed into a `.bin` file in this directory. This is so that the CLI can then simply get the runtime binary from the downloaded PyPI package. You can find more details in the `venice-cli` repository.
  -  [__init__.py](./venice/venice/__init__.py) Python stubs for the Venice API, including type annotations and docstrings. We generate docs from stubs using pdoc.
...
- [venice](./packages/venice): Code for the actual runtime.
  - [modvenice](./packages/venice/src/modvenice): code for the public-facing Venice Python API (which we try to keep the stubs up-to-date with).
...
Note that we use the `vexide_devices` crate from vexide (which is a port of Rust to the VEX V5) for a high-level abstraction; the vast majority of our Devices API simply mirrors the `vexide_devices` API. There are few Python-specific things that our API has to keep in mind (with few exceptions):
