...
static MOTOR_OBJ_TYPE: ObjFullType = ObjFullType::new(TypeFlags::empty(), qstr!(Motor))
    ...
    .set_slot_locals_dict_from_static(&const_dict![
        ...
        qstr!(set_gearset) => Obj::from_static(&fun2_from_fn!(fn motor_set_gearset(&MotorObj, &GearsetObj))),
        qstr!(gearset) => Obj::from_static(&fun1_from_fn!(fn motor_gearset(&MotorObj))),
        qstr!(set_position_target) => Obj::from_static(&fun_var_from_fn!(motor_set_position_target)),
        qstr!(is_exp) => Obj::from_static(&fun1_from_fn!(fn motor_is_exp(&MotorObj))),
        qstr!(is_v5) => Obj::from_static(&fun1_from_fn!(fn motor_is_v5(&MotorObj))),
        qstr!(max_voltage) => Obj::from_static(&fun1_from_fn!(fn motor_max_voltage(&MotorObj))),
        qstr!(velocity) => Obj::from_static(&fun1_from_fn!(fn motor_velocity(&MotorObj))),
        qstr!(power) => Obj::from_static(&fun1_from_fn!(fn motor_power(&MotorObj))),
        qstr!(torque) => Obj::from_static(&fun1_from_fn!(fn motor_torque(&MotorObj))),
        qstr!(voltage) => Obj::from_static(&fun1_from_fn!(fn motor_voltage(&MotorObj))),
        qstr!(raw_position) => Obj::from_static(&fun1_from_fn!(fn motor_raw_position(&MotorObj))),
        qstr!(current) => Obj::from_static(&fun1_from_fn!(fn motor_current(&MotorObj))),
        qstr!(efficiency) => Obj::from_static(&fun1_from_fn!(fn motor_efficiency(&MotorObj))),
        qstr!(current_limit) => Obj::from_static(&fun1_from_fn!(fn motor_current_limit(&MotorObj))),
        qstr!(voltage_limit) => Obj::from_static(&fun1_from_fn!(fn motor_voltage_limit(&MotorObj))),
        qstr!(temperature) => Obj::from_static(&fun1_from_fn!(fn motor_temperature(&MotorObj))),
        qstr!(set_profiled_velocity) => Obj::from_static(&fun2_from_fn!(fn motor_set_profiled_velocity(&MotorObj, i32))),
        qstr!(reset_position) => Obj::from_static(&fun1_from_fn!(fn motor_reset_position(&MotorObj))),
        qstr!(set_current_limit) => Obj::from_static(&fun2_from_fn!(fn motor_set_current_limit(&MotorObj, f32))),
        qstr!(set_voltage_limit) => Obj::from_static(&fun2_from_fn!(fn motor_set_voltage_limit(&MotorObj, f32))),
        qstr!(is_over_temperature) => Obj::from_static(&fun1_from_fn!(fn motor_is_over_temperature(&MotorObj))),
        qstr!(is_over_current) => Obj::from_static(&fun1_from_fn!(fn motor_is_over_current(&MotorObj))),
        qstr!(is_driver_fault) => Obj::from_static(&fun1_from_fn!(fn motor_is_driver_fault(&MotorObj))),
        qstr!(is_driver_over_current) => Obj::from_static(&fun1_from_fn!(fn motor_is_driver_over_current(&MotorObj))),
        qstr!(status) => Obj::from_static(&fun1_from_fn!(fn motor_status(&MotorObj))),
        qstr!(faults) => Obj::from_static(&fun1_from_fn!(fn motor_faults(&MotorObj))),
        qstr!(motor_type) => Obj::from_static(&fun1_from_fn!(fn motor_motor_type(&MotorObj))),
        qstr!(position) => Obj::from_static(&fun2_from_fn!(fn motor_position(&MotorObj, &RotationUnitObj))),
        qstr!(set_position) => Obj::from_static(&fun3_from_fn!(fn motor_set_position(&MotorObj, f32, &RotationUnitObj))),
        qstr!(set_direction) => Obj::from_static(&fun2_from_fn!(fn motor_set_direction(&MotorObj, &DirectionObj))),
        qstr!(direction) => Obj::from_static(&fun1_from_fn!(fn motor_direction(&MotorObj))),
    ]);
